...
int getJointAngle(){
  int angle = map(analogRead(pot_in), 0, 1023, -30, 210);
...
void loop() {
  ...
  if(mode == "velocity"){
    ...
    if (angle >= tPos+10){
      if (moving == CCW){
        ...
        // Moving down
        motorSpeed(CW, 10); //angle-tPos+10
        moving = CW;
  ...
  pos = map(1023-read, 0, 1023, -30, 210);
  int vel = (pos - prevPos)/((time - prevTime)/1000);
 // Serial.print(String(int(pos)) + sepperator + String(vel) + sepperator);
  Serial.println(read);
  delay(waitTime);
}
